...
import { removeSelectedSubject } from "../utils/remove-selected-subject.js";
 ...
 */
export const setMainContent = async (dataContext: DataContext): Promise<HTMLElement | undefined> => {
  try {
    const { context } = dataContext;
    const whiteSpace = document.createTextNode(" ");
    const main = document.createElement("main");
    if (context === "score") {
      const { subject, title } = dataContext;
      const [score, maxScore] = getScore(subject);
      main.id = "score";
      const header = document.createElement("header");
      const h2 = document.createElement("h2");
      h2.textContent = "Quiz completed";
      const headerP = document.createElement("p");
      headerP.textContent = "You scored…";
      ...
      img.src = `./images/icon-${subject}.svg`;
      img.alt = "";
      img.width = 56;
      img.height = 56;
      subjectSpan.appendChild(img);
      subjectSpan.appendChild(whiteSpace);
      subjectSpan.appendChild(document.createTextNode(title));
      const scoreNote = document.createElement("span");
      scoreNote.className = "score-note";
      scoreNote.textContent = String(score);
      resultP.appendChild(subjectSpan);
      resultP.appendChild(whiteSpace);
      resultP.appendChild(scoreNote);
      resultP.appendChild(whiteSpace);
      resultP.appendChild(document.createTextNode(`out of ${maxScore}`));
